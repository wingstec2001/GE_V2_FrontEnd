...
export default {
  ...
  methods: {
    ...
    loadData(selMonth) {
      console.log('loadData,selMonth:', selMonth)
      this.loadMthlyMatStock(selMonth).then(() => {
        this.filterMthlyMatStock(this.material_id)
        this.materialIds = this.MthlyMatStock.map(fs => fs.material_id)
        this.$nextTick(() => {
          this.$refs.MthlyMatStock.toggleSelectAll(true)
        })

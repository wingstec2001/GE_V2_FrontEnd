...
export default {
  ...
  computed: {
    ...mapState('getsujiCrushedStore', ['selMonthlyCrushed']),
    weightErrors() {
      const errors = []
  ...
  methods: {
    ...mapActions('getsujiCrushedStore', ['loadMonthlyCrushed', 'editMonthlyCrushed']),
    ...
    submit(item) {
      ...
      if (!this.$v.$invalid) {
        ...
        this.editMonthlyCrushed(submitForm).then(() => {
    ...
    openForm() {
      ...
      this.loadMonthlyCrushed(this.id).then(() => {
        this.item = { ...this.selMonthlyCrushed }
        this.dialog = true
        this.setOverlayStatus(false)
        console.log('loadMonthlyCrushed', this.selMonthlyCrushed)
      })
    },
    closeForm() {
      this.$emit('cancel')

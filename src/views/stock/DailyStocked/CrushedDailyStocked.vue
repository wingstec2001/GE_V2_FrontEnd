...
import CrushedMthInboundEdit from './CrushedMthInboundEdit.vue'
import CrushedDailyInbound from './CrushedDailyInbound.vue'
import CrushedDailyOutbound from './CrushedDailyOutbound.vue'
/* eslint-disable */
export default {
  components: {
    CrushedMthInboundEdit,
  ...
  computed: {
    ...mapState('dailyStockStore', ['DailyCrushedStock', 'wgtin_lmSum', 'wgtout_lmSum', 'net_weight','getsuji_id']),
    selectedMonth() {
    ...
    MthWgtIn() {
      let sum = 0
      if (this.DailyCrushedStock) {
        this.DailyCrushedStock.forEach(cp => {
          sum += cp.weight_in
        })
      }
  ...
  methods: {
    ...mapActions('dailyStockStore', ['loadDailyCrushedInfo','recalculateCrushedStock']),

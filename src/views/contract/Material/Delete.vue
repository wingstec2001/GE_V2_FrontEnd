...
export default {
  ...
  computed: {
    ...mapState('contractStore', ['selectedMaterialContract']),
    cm_id: {
      get() {
        return `CM-${this.contractId.toString()}`
      },

    },
  },
  methods: {
    ...mapActions('contractStore', ['loadMaterialContract', 'deleteMaterialContract']),
    ...mapActions('materialStore', ['loadMaterialIds']),
    ...mapActions('customerStore', ['loadCustomerIds']),

    openForm() {
      console.log('openForm,id:', this.contractId)
      this.loadMaterialContract(this.contractId).then(() => {
        this.item = { ...this.selectedMaterialContract }
        console.log('this.item:', this.items, this.selectedMaterialContract)
      })
      this.dialog = true
    },
    closeForm() {
    ...
    deleteItem(item) {
      ...
      this.deleteMaterialContract(item.id).then(() => {
        this.dialog = false

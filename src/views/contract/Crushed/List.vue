<template>
  <div id="contrated-list">
    <v-card>
      <v-card-title>契約(粉砕済) ― 一覧 </v-card-title>
      ...
      <v-card-text class="d-flex align-center flex-wrap pb-0">
        ...
        <div class="d-flex align-center flex-wrap">
          <v-btn
            color="primary"
            class="mb-4 me-3"
            :to="{ name: 'contract-crushed-create', params: { query: this.$route.query } }"
          >
            <span>NEW</span>
          </v-btn>
          <v-btn
...
export default {
  ...
  computed: {
    ...
    ...mapState('contractStore', ['crushedContracts', 'f_contracts', 'contractIds']),
    sumWgt() {
      let sum = 0
  ...
  methods: {
    ...
    ...mapActions('contractStore', ['loadCrushedContracts', 'deleteContract', 'loadContractIds']),
    ...mapMutations('app', ['setOverlayStatus']),

    loadData(status) {
      this.setOverlayStatus(true)
      this.loadCrushedContracts(this.$route.query).then(() => {
        this.contractList = [...this.crushedContracts]
    ...
    editItem(item) {
      ...
      this.$router.push({ name: 'contract-crushed-edit', params: { id:item.did, query: this.$route.query } })
    },
    ...
    downloadCSV() {
      ...
      this.contractList.forEach(el => {
        console.log("detail_id:",el.detail_id)
        no += 1
        const contract_id =`CF-${el.contract_id}-${el.detail_id}(${el.did})`
      ...
      link.download = `粉砕済契約一覧-${newstr}.csv`
      link.click()
    },
    getConfirmURL(item) {
      console.log("getConfirmURL,id:",item.contract_id)
      const ret = './crushed-confirm/' + item.contract_id
      return ret
    },

    getContractID(item) {
      ret = "CF"+ item.contract_id.toString().padStart(6,0)+item.detail_id.toString()
      return ret
    }
  },
}

<template>
  <v-card>
    <v-card-title>生産計画-編集</v-card-title>
    <template>
      <v-spacer></v-spacer>
        ...
        <v-form class="ma-6">
          <v-row align="center">
            <v-col
              ...
              md="2"
            >
              <label for="plan_date">計画日<span style="color: red">*</span></label>
            </v-col>
            <v-col
              cols="12"
              md="4"
...
export default {
  ...
  created() {
    this.loadProductIds()
    this.loadProductionPlan(this.$route.params.id).then(() => {
      this.ProductionPlanForm = { ...this.selectedProductionPlan }
      this.plan_dt = this.ProductionPlanForm.plan_date.substring(0, 10)
      this.plan_hour = this.ProductionPlanForm.plan_date.substring(11, 13)
      this.plan_minute = this.ProductionPlanForm.plan_date.substring(14, 16)
      this.ProductionPlanForm.route_id = this.ProductionPlanForm.route_id.toString()
      this.bind_weight = this.ProductionPlanForm.plan_weight
    })
  },
  methods: {
    focus(event) {
    ...
    submit(ProductionPlanForm) {
      ...
      if (!this.$v.$invalid) {
        ...
        this.editProductionPlan(ProductionPlanForm)
          .then(() => {
    ...
    ...mapActions('productionplanStore', ['loadProductionPlan', 'editProductionPlan']),
  },
}
</script>

<template>
  <v-card>
    <v-card-title>権限-新規</v-card-title>
    <template>
      <v-spacer></v-spacer>
...
export default {
  ...
  computed: {
    ...mapState('roleStore', ['createRoleInfo']),
    nameErrors() {
      const errors = []
  ...
  created() {
    this.loadCreateRoleInfo().then(() => {
      this.permissions = this.createRoleInfo.permissions
    })
  },
  methods: {
    jumpToListPage() {
    ...
    ...mapActions('roleStore', ['createRole', 'loadCreateRoleInfo']),
  },
}
</script>

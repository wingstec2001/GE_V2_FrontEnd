<template>
  <div id="aad-list">
    <v-card>
      <v-card-title>材料入荷(一覧)</v-card-title>
      <v-row class="px-2 ma-0">
        ...
        >
          <v-select
            v-model="crushing_status"
            :items="crushingStatuses"
            placeholder="粉砕状態"
            outlined
            clearable
            dense
      ...
      >
        ...
        >
          {{ item.aad_id }}({{ item.arrival_id }}) <!-- {{ getArrivalId( item ) }} -->
        </template>
        <template
...
export default {
  ...
  computed: {
    ...
    ...mapState('arrivalactualStore', ['selectedOrderByDetail']),
    ...
    sumWeight() {
      let sum = 0
      this.arrivalDetailsList.forEach(a => {
  ...
  methods: {
    ...
    loadData() {
      ...
      this.loadArrivalDetails(this.$route.query).then(() => {
    ...
    editItem(item) {
      ...
      this.$router.push({ name: 'arrivalmaterial-edit', params: { id: item.aad_id, query: this.$route.query } })
    },
    detailItem(arrivalId) {
      this.$router.push({ name: 'arrivalmaterial-detail', params: { id: arrivalId, query: this.$route.query } })
    },
    ...
    loadQuery() {
      ...
      this.crushing_status = this.$route.query.crushing_status ?? ''
    ...
    downloadCSV(arrivalId) {
      ...
      sorted.forEach(el => {
        ...
        const note = el.note ?? ''
        const line = `${no},"${el.aad_id}","${el.arrival_date}","${el.material_id}","${status}","${weight}","${customer_id}","${note}"\n`
        csv += line
      })
